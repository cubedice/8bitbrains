...
catch {set coef_files [glob -nocomplain filter2_coef*.txt]} error_msg
puts "$error_msg"
  
# The top-level in HDL type "VHDL"
set ipfs_ext vho
if {[file exists filter2.vho]} {
	set hdl_ext vhd
} else {
	puts "Warning: Could not find filter2.$ipfs_ext!"
}
...
set_global_assignment -name EDA_TEST_BENCH_MODULE_NAME work.tb_filter2 -section_id tb
set_global_assignment -name EDA_TEST_BENCH_GATE_LEVEL_NETLIST_LIBRARY work -section_id tb

# IPFS file
set_global_assignment -name EDA_IPFS_FILE filter2.$ipfs_ext -section_id eda_simulation -library work
...
set_global_assignment -name EDA_TEST_BENCH_FILE filter2_input.txt -section_id tb -library work

set_global_assignment -name EDA_TEST_BENCH_FILE tb_filter2.vhd -section_id tb -library work
